...
.controller('TeamsView', ['$scope', '$interval', 'teamService', function($scope,$interval, teamService) {
  teamService.setGroupWinners(undefined);
  teamService.setWinner(undefined); 
  $scope.show = false;
  ...
  teamService.getTeamMembers().get(function(data){
    $scope.teams = Array(32).fill().map((team, i)=>{
      team = { id: i, name: $scope.teamNames[i], members: Array(11), img: deCapitalizeFirstLetter($scope.teamNames[i].replace(' ','')) + '.png', win: false, score: 0, semiScore: 0, goals: 0};
  ...
  $scope.restartInterval = function(){
    ...
    setTimeout(() => document.getElementById('groupHeader').scrollIntoView({block: 'start', behavior: 'smooth'}), 0);
